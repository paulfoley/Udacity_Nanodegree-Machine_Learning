...
#### Output Number of People in Dataset
...
for data in data_dict:
	if data_dict[data]['salary'] != 'NaN' and data_dict[data]['bonus'] != 'NaN':
		if data_dict[data]['salary'] > 1000000:
			if data_dict[data]['bonus'] > 5000000 :
				print(data)

##### Returns Jeff Skilling and Ken Lay, the two top dogs,
##### Their salaries and bonuses definitely deviate from the norm


#################### Feature Creation ####################
#### Calculate the Fraction of From and To POI's as new features
def computeFraction(poi_messages, all_messages):
    """ 
    Given a number messages to/from POI (numerator) 
    and number of all messages to/from a person (denominator),
    Function returns the fraction of messages to/from that person that are from/to a POI.
    
    When "NaN", there is no known email address
    (and so no filled email features)
...
#### To Speed Up Testing we'll Use a Smaller Dataset
features_train = features_train[:150]
labels_train = labels_train[:150]

#### Use SelectKBest to get the Rop Features
...
#################### Create Classifiers ####################
'''
Try a variety of classifiers to see which one best identifies POI's
'''
